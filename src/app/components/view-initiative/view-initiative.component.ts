...
@Component({
  ...
  styleUrls: ['./view-initiative.component.css']
})
export class ViewInitiativeComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder,private _location:Location,private ws:WebserviceService,private ui:UIserviceService,private router:Router) {
    ...
    localStorage.setItem('selectedInitiative',JSON.stringify(sampleInitiative))
...
selectedInitiative
selectedgroupValues
selectedilstatusValues
selectedpotentialValues
selectedInitID 
  ...
  ngOnInit() {
    ...
    var selectedInitId = localStorage.getItem('selectedId')
    
    var geturlValue = "/DTAOneApp/initiative/get/" + selectedInitId

   

    this.ws.method(geturlValue, '', 'get')
      .subscribe(initData => {
      console.log(initData);
      this.selectedInitID = initData.id;
      this.selectedInitiative = initData
     

    //fetch the selected initiative from local
    // console.log(localStorage.getItem('selectedInitiative'));
    // var val = localStorage.getItem('selectedInitiative')
    // console.log(JSON.parse(val));
    // this.selectedInitiative = JSON.parse(val)
    // this.selectedInitiative = JSON.parse(localStorage.getItem('selectedInitiative'))
    console.log(this.selectedInitiative)
    
    // groupvalues: this.fb.array([]),
    // ilstatus: this.fb.array([]),
    // potentials: this.fb.array([]),
    //get all the stringifid values
    
    // this.selectedgroupValues = JSON.parse(this.selectedInitiative['groupvalues'])
    // this.selectedilstatusValues = JSON.parse(this.selectedInitiative['ilstatus'])
    // this.selectedpotentialValues = JSON.parse(this.selectedInitiative['potentials'])
    ...
    this.initiativeForm.patchValue(this.selectedInitiative)


    console.log(this.initiativeForm.value)
    var dispTag = [];
    var tagJson = JSON.parse(this.initiativeForm.value['tags'])
    tagJson.map((item)=>{
      dispTag.push(item['item_text'])
    })

    this.elementsList = [
      this.ui.formValue("Initiative Title", "title", 12, "label",this.initiativeForm.value['title']),
      this.ui.formValue("Description", "description", 12, "label",this.initiativeForm.value['description']),
      this.ui.formValue("Tags", "tags", 12, "label",dispTag),
    ]
    ...
    Promise.all(promiseList).then((dataArr)=>{
      ...
      this.elementsList2 = [
        this.ui.formValue("Horizontal Pillar", "horipillarid", 6, "label", this.getValues(dataArr[0],"horipillarid")),
        this.ui.formValue("Vertical Pillar", "vertpillarid", 6, "label", this.getValues(dataArr[1],"vertpillarid")),
        // this.ui.formValue("Financial Impact", "finimpactid", 4, "label", getValues(dataArr[2],"finimpactid")),
        this.ui.formValue("Area", "areaid", 4, "label", this.getValues(dataArr[2],"areaid")),
        this.ui.formValue("Currency", "currencyid", 4, "label",this.getValues(dataArr[3],"currencyid")),
      ]
    })
    ...
    this.elementsList3 = [
      this.ui.formValue("L2", "respl2", 4, "label", this.initiativeForm.value['respl2']),
      this.ui.formValue("L3", "respl3", 4, "label", this.initiativeForm.value['respl3']),
      this.ui.formValue("L4", "respl4", 4, "label", this.initiativeForm.value['respl4']),
    ]
  ...
  getValues(inputArr,cname){
    var hValue
    inputArr.map((data)=>{
      console.log(data);
      console.log(cname)
      console.log(this.selectedInitiative[cname]);
      if(this.selectedInitiative[cname] == ""+data['id']){
        hValue = data['name']
      }
  ...
  private createForm() {
    ...
    this.initiativeForm.patchValue(this.initiativeForm)
  }
  ...
  addComment(){
    ...
    submitData["id"] = this.selectedInitID
